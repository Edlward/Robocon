...
void Phase_Change(Phase_State *target,float speed){
  Phase OtherPhase = (Phase)(3-target->Low-target->High);
  Set_Phase_Close(OtherPhase);
  Phase_Open_Cnt=0;
  //Set_Phase_Low_State(target->Low,LOW_OPEN); //TODO
  Set_Phase_High_Speed(target->Low,0); 
...
uint16_t Read_Mag(){
  uint8_t data[2];
  uint16_t position;
  HAL_GPIO_WritePin(CSN_MAG_GPIO_Port,CSN_MAG_Pin,GPIO_PIN_RESET);
