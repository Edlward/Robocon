...
#define STM32_SPI_USE   hspi1

#define NRF_SPI_Write_Read(write_ptr,read_ptr,len)   HAL_SPI_TransmitReceive(&STM32_SPI_USE,write_ptr,read_ptr,len,50) 
#define NRF_SPI_Read(read_ptr,len) HAL_SPI_Receive(&STM32_SPI_USE,read_ptr,len,50)
#define NRF_SPI_Write(write_ptr,len) HAL_SPI_Transmit(&STM32_SPI_USE,write_ptr,len,50)
...
uint8_t NRF_Init(uint8_t rx_channel_num){
...
REDO:
  ...
  result&=NRF_Write_Reg(RF_SETUP,0x26); //2mhz 0db
  
  if(rx_channel_num>5){
    return 0;

  }
  NRF_RX_CHANNEL_NUM=rx_channel_num;
  uint8_t temp=0;
  ...
  //for(int i=0;i<2;++i){
    //result&=NRF_Write_Bytes(RX_ADDR_P0 + i,RX_ADDRESS[i],sizeof(RX_ADDRESS[0]));
  //}

  //result&=NRF_Write_Bytes(RX_ADDR_P0 + 1,RX_ADDRESS[1],sizeof(RX_ADDRESS[0]));
...
uint8_t NRF_Set_Rx_Addr(uint8_t rx_addrs[][5],size_t size_check){
  ...
  */
  ...
  for(int i=0;i<2&&i<NRF_RX_CHANNEL_NUM;++i){
    result&=NRF_Write_Bytes(RX_ADDR_P0 + i,rx_addrs[i],sizeof(RX_ADDRESS[0]));
  }
  for(int i=0;i<4&&NRF_RX_CHANNEL_NUM>2;++i){
...
void NRF_Receive_IT(){
  ...
  NRF_Set_GPIO(CE,HIGH);
  NRF_Set_Mode(NRF_RX,CRC_ENABLE); 
  NRF_Write_Reg(STATUS,0x40);  //清空STATUS
}
